...
(() => {
  ...
  var CtTimer = class {
    ...
    constructor(timeMs, name, uiDelta = false) {
      ...
      if (!rooms_default.current) {
        throw new Error(
          "[CtTimer] Timers can only be created when a main room exists. If you need to run async tasks before ct.js game starts, use vanilla JS' setTimeout method."
        );
      }
    ...
    update() {
      ...
      if (
        !rooms_default.current ||
        (rooms_default.current.uid !== this[ctTimerRoomUid] &&
  ...
  var playVariant = (sound, variant, options) => {
    const pixiSoundInst = PIXI.sound
      .find(`${pixiSoundPrefix}${variant.uid}`)
      .play(options);
    if (sound.volume?.enabled) {
      pixiSoundInst.volume =
        randomRange(sound.volume.min, sound.volume.max) *
        (options?.volume || 1);
  ...
  var playRandomVariant = (sound, options) => {
    const variant =
      sound.variants[Math.floor(Math.random() * sound.variants.length)];
  ...
  var soundsLib = {
    ...
    play(name, options) {
      ...
      throw new Error(
        `[sounds.play] Sound "${name}" was not found. Is it a typo?`
      );
    },
    /**
    ...
    playing(name) {
      ...
      if (name in pixiSoundInstances) {
        ...
        for (const variant of soundMap[name].variants) {
          ...
          if (
            instanceKey in pixiSoundInstances &&
        ...
        throw new Error(
          `[sounds] Sound "${name}" was not found. Is it a typo?`
        );
      }
      return false;
    },
    ...
    addEqualizer(sound, f32, f64, f125, f250, f500, f1k, f2k, f4k, f8k, f16k) {
      const fx = new PIXI.sound.filters.EqualizerFilter(
        f32,
        f64,
        f125,
        f250,
    ...
    removeFilter(name, filter) {
      ...
      if (!name) {
        ...
        filters =
          typeof name === "string"
            ? withSound(name, (soundInst) => soundInst.filters)
            : name.filters;
      }
    ...
    speed(name, value) {
      ...
      if (typeof name === "string") {
        ...
        throw new Error(
          `[sounds.speed] Invalid sound name: ${name}. Is it a typo?`
        );
      }
      return name.speed;
    },
  ...
  var getEntriesByPath = (nPath) => {
    if (!resLib.tree) {
      throw new Error(
        "[res] Asset tree was not exported; check your project's export settings."
      );
    }
    let current = resLib.tree;
    for (const subpath of nPath) {
      const folder = current.find(
        (i) => i.name === subpath && i.type === "folder"
      );
      if (!folder) {
        throw new Error(
          `[res] Could not find folder ${subpath} in path ${nPath.join("/")}`
        );
      }
  ...
  var resLib = {
     ...
     */
    async loadTexture(
      url = required("url", "ct.res.loadTexture"),
      name = required("name", "ct.res.loadTexture"),
      textureOptions = {}
    ) {
      let texture;
      try {
    ...
    async unloadAtlas(url = required("url", "ct.res.unloadAtlas")) {
      ...
      if (!animations) {
        console.log(
          `[ct.res] Attempt to unload an atlas that was not loaded/was unloaded already: ${url}`
        );
        return;
      }
     ...
     */
    loadSound(
      path = required("path", "ct.res.loadSound"),
      name = required("name", "ct.res.loadSound"),
      preload = true
    ) {
    ...
    async loadGame() {
      ...
      const atlases = [["./img/a0.{webp,png}.021feea8be.json"]][0];
      const tiledImages = [
        {
          bg: {
            source: "./img/t0.68812d0682.{webp,png}",
            shape: { type: "rect", top: 0, bottom: 1080, left: 0, right: 1920 },
            anchor: { x: 0, y: 0 },
          },
        },
      ][0];
      ...
      loadingPromises.push(
        ...atlases.map((atlas) =>
          resLib.loadAtlas(atlas).then((texturesNames) => {
            assetsLoaded++;
      ...
      for (const name in tiledImages) {
        loadingPromises.push(
          resLib.loadTexture(tiledImages[name].source, name, {
    ...
    getTextureShape(name) {
      ...
      if (!(name in resLib.textures)) {
        throw new Error(
          `Attempt to get a shape of a non-existent texture ${name}`
        );
      }
     ...
     */
    getChildren(path) {
      return getEntriesByPath(normalizeAssetPath(path || "")).filter(
  ...
  var Background = class extends PIXI.TilingSprite {
    constructor(
      ...
      const texture =
        texName instanceof PIXI.Texture
          ? texName
          : res_default.getTexture(texName, frame || 0);
      ...
      stack.push(this);
      this.on("destroyed", () => {
        templates_default.list.BACKGROUND.splice(
          templates_default.list.BACKGROUND.indexOf(this),
          1
        );
    ...
    reposition() {
      ...
      const dx = camera_default.x - camera_default.width / 2,
        dy = camera_default.y - camera_default.height / 2;
  ...
  var backgroundsLib = {
    ...
    add(texName, frame = 0, depth = 0, container) {
      if (!rooms_default.current) {
        throw new Error(
          "[backgrounds.add] Cannot add a background before the main room is created"
        );
      }
      if (!container) {
  ...
  var Tilemap = class extends PIXI.Container {
    ...
    constructor(template) {
      ...
      this.on("destroyed", () => {
        templates_default.list.TILEMAP.splice(
          templates_default.list.TILEMAP.indexOf(this),
          1
        );
      });
    }
    ...
    cache(chunkSize = 1024) {
      if (this.cached) {
        throw new Error(
          "[ct.tiles] Attempt to cache an already cached tilemap."
        );
      }
    ...
    cacheDiamond(chunkSize = 1024) {
      ...
      for (let i = 0, l = this.tiles.length; i < l; i++) {
        const [tile] = this.children;
        const { x: xNormalized, y: yNormalized } = u_default.rotate(
          tile.x,
          tile.y * 2,
          -45
        );
  ...
  var tilemapsLib = {
    ...
    create(depth = 0) {
      if (!rooms_default.current) {
        throw new Error(
          "[emitters.fire] An attempt to create a tilemap before the main room is created."
        );
      }
  ...
  var behaviorsLib = {
    ...
    add(target, behavior) {
      if (target.behaviors.includes(behavior)) {
        throw new Error(
          `[behaviors.add] Behavior ${behavior} already exists on ${
            target instanceof Room ? target.name : target.template
          }`
        );
      }
      ...
      if (bh === "static") {
        throw new Error(
          `[behaviors.add] Behavior ${behavior} cannot be added to ${
            target instanceof Room ? target.name : target.template
          } because this behavior cannot be added dynamically.`
        );
      }
      if (!bh) {
        throw new Error(
          `[behaviors.add] Behavior ${behavior} does not exist or cannot be applied to ${domain}.`
        );
      }
    ...
    remove(target, behavior) {
      if (!target.behaviors.includes(behavior)) {
        throw new Error(
          `[behaviors.remove] Behavior ${behavior} already exists on ${
            target instanceof Room ? target.name : target.template
          }`
        );
      }
      ...
      if (bh === "static") {
        throw new Error(
          `[behaviors.remove] Behavior ${behavior} cannot be removed from ${
            target instanceof Room ? target.name : target.template
          } because this behavior cannot be removed dynamically.`
        );
      }
      if (!bh) {
        throw new Error(
          `[behaviors.remove] Behavior ${behavior} does not exist or cannot be applied to ${domain}.`
        );
      }
      if (bh.thisOnRemoved) {
  ...
  var PixiButton = class extends PIXI.Container {
    ...
    constructor(t, exts) {
      if (t?.baseClass !== "Button") {
        throw new Error(
          "Don't call PixiButton class directly! Use templates.copy to create an instance instead."
        );
      }
      super();
      this.normalTexture = res_default.getTexture(t.texture, 0);
      this.hoverTexture = t.hoverTexture
        ? res_default.getTexture(t.hoverTexture, 0)
        : this.normalTexture;
      this.pressedTexture = t.pressedTexture
        ? res_default.getTexture(t.pressedTexture, 0)
        : this.normalTexture;
      this.disabledTexture = t.disabledTexture
        ? res_default.getTexture(t.disabledTexture, 0)
        : this.normalTexture;
      ...
      const style =
        t.textStyle === -1
          ...
          : styles_default.get(t.textStyle, true);
      if (exts.customSize) {
  ...
  var PixiSpritedCounter = class extends PIXI.TilingSprite {
    ...
    constructor(t, exts) {
      if (t.baseClass !== "SpritedCounter") {
        throw new Error(
          "Don't call PixiScrollingTexture class directly! Use templates.copy to create an instance instead."
        );
      }
  ...
  var PixiTextBox = class extends PIXI.Container {
    ...
    constructor(t, exts) {
      if (t?.baseClass !== "TextBox") {
        throw new Error(
          "Don't call PixiTextBox class directly! Use templates.copy to create an instance instead."
        );
      }
      super();
      ...
      this.#repositionRestyleInput = () => {
        ...
        const scalar = isUi
            ? u_default.uiToCssScalar
            : u_default.gameToCssScalar,
          coord = isUi ? u_default.uiToCssCoord : u_default.gameToCssCoord;
        ...
        Object.assign(this.#htmlInput.style, {
          ...
          color: Array.isArray(textStyle.fill)
            ? textStyle.fill[0]
            : textStyle.fill,
        });
        if (textStyle.strokeThickness) {
          this.#htmlInput.style.textStroke = `${scalar(
            textStyle.strokeThickness / 2
          )}px ${textStyle.stroke}`;
          this.#htmlInput.style.webkitTextStroke =
            this.#htmlInput.style.textStroke;
        } else {
          this.#htmlInput.style.textStroke =
            this.#htmlInput.style.webkitTextStroke = "unset";
        }
        if ("dropShadow" in textStyle) {
          const angle = u_default.radToDeg(textStyle.dropShadowAngle ?? 0);
          let x = u_default.ldx(textStyle.dropShadowDistance ?? 0, angle),
            y = u_default.ldy(textStyle.dropShadowDistance ?? 0, angle);
          x = scalar(x);
          y = scalar(y);
          const css = `${x}px ${y}px ${scalar(
            textStyle.dropShadowBlur ?? 0
          )}px ${textStyle.dropShadowColor}`;
    ...
    #setFocused(val) {
      ...
      if (val) {
        ...
        this.panel.texture =
          this.pressedTexture ?? this.hoverTexture ?? this.normalTexture;
        this.#repositionRestyleInput();
        if (this.maxLength > 0) {
  ...
  var PixiPanel = class extends PIXI.NineSlicePlane {
    constructor(t, exts) {
      if (t?.baseClass !== "NineSlicePlane") {
        throw new Error(
          "Don't call PixiPanel class directly! Use templates.copy to create an instance instead."
        );
      }
      const tex = res_default.getTexture(t.texture, 0);
      super(
  ...
  var PixiText = class extends PIXI.Text {
    constructor(t, exts) {
      if (t?.baseClass !== "Text") {
        throw new Error(
          "Don't call PixiText class directly! Use templates.copy to create an instance instead."
        );
      }
      let style;
      ...
      super(exts.customText || t.defaultText || "", style);
      ...
      this.scale.set(exts.scaleX ?? 1, exts.scaleY ?? 1);
      return this;
    }
  };
  ...
  var PixiBitmapText = class extends PIXI.BitmapText {
    constructor(t, exts) {
      if (t?.baseClass !== "BitmapText") {
        throw new Error(
          "Don't call PixiBitmapText class directly! Use templates.copy to create an instance instead."
        );
      }
      let style;
      if (t.textStyle && t.textStyle !== -1) {
      ...
      super(exts.customText || t.defaultText || "", {
        ...style,
        fontName: style.fontFamily.split(",")[0].trim(),
  ...
  var CopyProto = {
    ...
    get direction() {
      ...
      return (
        ((Math.atan2(this.vspeed, this.hspeed) * 180) / Math.PI + 360) % 360
      );
    },
    set direction(value) {
  ...
  var Copy = function (x, y, template, container, exts) {
    ...
    if (template) {
      ...
      if (
        template.baseClass === "AnimatedSprite" ||
        template.baseClass === "NineSlicePlane"
      ) {
  ...
  var makeCopy = (template, x, y, parent, exts) => {
    if (!(template in templatesLib.templates)) {
      throw new Error(
        `[ct.templates] An attempt to create a copy of a non-existent template \`${template}\` detected. A typo?`
      );
    }
    const t = templatesLib.templates[template];
    if (!(t.baseClass in baseClassToPixiClass)) {
      throw new Error(
        `[internal -> makeCopy] Unknown base class \`${t.baseClass}\` for template \`${template}\`.`
      );
    }
  ...
  var templatesLib = {
    ...
    copy(template, x = 0, y = 0, params = {}) {
      if (!rooms_default.current) {
        throw new Error(
          "[emitters.fire] An attempt to create a copy before the main room is created."
        );
      }
      return templatesLib.copyIntoRoom(
        template,
        x,
        y,
        rooms_default.current,
    ...
    copyIntoRoom(template, x = 0, y = 0, room, params = {}) {
      if (!room || !(room instanceof Room)) {
        throw new Error(
          `Attempt to spawn a copy of template ${template} inside an invalid room. Room's value provided: ${room}`
        );
      }
    ...
    exists(template) {
      if (!(template in templatesLib.templates)) {
        throw new Error(
          `[ct.templates] templates.exists: There is no such template ${template}.`
        );
      }
...
Shape: ${(this._shape && this._shape.__type) || "unused"}`;
  ...
  var shakeCamera = function shakeCamera2(camera2, time) {
    ...
    camera2.shakePhaseX +=
      phaseDelta * (1 + Math.sin(camera2.shakePhase * 0.1489) * 0.25);
    camera2.shakePhaseY +=
      phaseDelta * (1 + Math.sin(camera2.shakePhase * 0.1734) * 0.25);
  };
  var followCamera = function followCamera2(camera2) {
    const bx =
        camera2.borderX === null
          ? camera2.width / 2
          : Math.min(camera2.borderX, camera2.width / 2),
      by =
        camera2.borderY === null
          ? camera2.height / 2
          : Math.min(camera2.borderY, camera2.height / 2);
    const tl = camera2.uiToGameCoord(bx, by),
      br = camera2.uiToGameCoord(camera2.width - bx, camera2.height - by);
    if (!camera2.follow) {
      return;
    }
    if (camera2.followX) {
  ...
  var Camera = class extends PIXI.DisplayObject {
    ...
    update(time) {
      ...
      this.interpolatedShiftX =
        this.shiftX * speed + this.interpolatedShiftX * (1 - speed);
      this.interpolatedShiftY =
        this.shiftY * speed + this.interpolatedShiftY * (1 - speed);
      restrictInRect(this);
      this.x = this.x || 0;
      this.y = this.y || 0;
    ...
    get computedX() {
      const dx =
        (Math.sin(this.shakePhaseX) +
          Math.sin(this.shakePhaseX * 3.1846) * 0.25) /
        1.25;
      const x =
        this.x +
        ((dx * this.shake * Math.max(this.width, this.height)) / 100) *
          this.shakeX;
    ...
    get computedY() {
      const dy =
        (Math.sin(this.shakePhaseY) +
          Math.sin(this.shakePhaseY * 2.8948) * 0.25) /
        1.25;
      const y =
        this.y +
        ((dy * this.shake * Math.max(this.width, this.height)) / 100) *
          this.shakeY;
    ...
    realign(room) {
      if (!room.isUi) {
        ...
        throw new Error(
          "[ct.camera] An attempt to realing a room that is not in UI space."
        );
      }
      const w = rooms_default.templates[room.name].width || 1,
        h = rooms_default.templates[room.name].height || 1;
  ...
  var mainCamera = new Camera(0, 0, [1280][0], [720][0]);
  ...
  var uLib = {
    ...
    rotateRad(x, y, rad) {
      const sin = Math.sin(rad),
        cos = Math.cos(rad);
      return new PIXI.Point(cos * x - sin * y, cos * y + sin * x);
    },
    /**
    ...
    reshapeNinePatch(copy) {
      ...
      if (origShape.type === "circle") {
        throw new Error(
          `[u.reshapeNinePatch] Cannot reshape a circular collision mask for ${copy.template}. Please use a different collision type for its texture.`
        );
      }
      if (origShape.type === "rect") {
        ...
        if (origShape.right < target.rightWidth) {
          shape.right =
            ((origShape.right - target.rightWidth) * (1 + dwx)) / bw;
        }
        ...
        if (origShape.bottom < target.bottomHeight) {
          shape.bottom =
            ((origShape.bottom - target.bottomHeight) * (1 + dhy)) / bh;
        }
        copy.shape = shape;
        return;
      }
      if (origShape.type === "strip") {
        ...
        shape.points = origShape.points.map((point) => {
          ...
          if (point.x >= origWidth - target.rightWidth) {
            ...
            x =
              target.leftWidth + (point.x - target.leftWidth) * (1 + dwx / bw);
          }
          if (point.y >= origHeight - target.bottomHeight) {
            y += dhy;
          } else if (point.y > target.topHeight) {
            y =
              target.topHeight + (point.y - target.topHeight) * (1 + dhy / bh);
          }
          return {
            x,
            y,
    ...
    getHitArea(shape) {
      ...
      if (shape.type === "strip") {
        return new PIXI.Polygon(
          shape.points.map((point) => new PIXI.Point(point.x, point.y))
        );
      }
      return false;
    },
    ...
    prect(x, y, arg) {
      ...
      if (arg instanceof Array) {
        ...
        if (arg.shape.type !== "rect") {
          throw new Error(
            "[ct.u.prect] The specified copy doesn't have a rectangular collision shape."
          );
        }
    ...
    pcircle(x, y, arg) {
      ...
      if (arg.shape.type !== "circle") {
        throw new Error(
          "[ct.u.pcircle] The specified copy doesn't have a circular shape"
        );
      }
      return (
        uLib.pdc(0, 0, (arg.x - x) / arg.scale.x, (arg.y - y) / arg.scale.y) <
        arg.shape.r
      );
    },
    /**
    ...
    gameToCssScalar(val) {
      return (
        (val / (camera_default.width * camera_default.scale.x)) *
        canvasCssPosition.width
      );
    },
    gameToUiCoord(x, y) {
  ...
  var Room = class _Room extends PIXI.Container {
    ...
    constructor(template, isRoot) {
      ...
      if (template) {
        ...
        if (isRoot) {
          ...
          pixiApp.renderer.background.color = u_default.hexToPixi(
    ...
    static realignElement(copy, oldWidth, oldHeight, newWidth, newHeight) {
      ...
      const wref =
          (oldWidth * (frame.x2 - frame.x1)) / 100 -
          padding.left -
          padding.right,
        href =
          (oldHeight * (frame.y2 - frame.y1)) / 100 -
          padding.top -
          padding.bottom;
      const xnew = (newWidth * frame.x1) / 100 + padding.left,
        ynew = (newHeight * frame.y1) / 100 + padding.top;
      const wnew =
          (newWidth * (frame.x2 - frame.x1)) / 100 -
          padding.left -
          padding.right,
        hnew =
          (newHeight * (frame.y2 - frame.y1)) / 100 -
          padding.top -
          padding.bottom;
  ...
  var roomsLib = {
    ...
    addBg(texture, depth) {
      if (!roomsLib.current) {
        throw new Error(
          "[rooms.addBg] You cannot add a background before a room is created"
        );
      }
    ...
    remove(room) {
      if (!(room instanceof Room)) {
        if (typeof room === "string") {
          console.error(
            "[rooms.remove] To remove a room, you should provide a reference to it (to an object), not its name. Provided value:",
            room
          );
          throw new Error("[rooms.remove] Invalid argument type");
        }
        throw new Error(
          "[rooms] An attempt to remove a room that is not actually a room! Provided value:" +
            room
        );
      }
      ...
      if (ind !== -1) {
        ...
        console.warn(
          "[rooms] Removing a room that was not found in rooms.list. This is strange\u2026"
        );
      }
      room.kill = true;
    ...
    restart() {
      if (!roomsLib.current) {
        throw new Error(
          "[rooms.restart] Cannot restart a room before it is created"
        );
      }
    ...
    append(roomName, params) {
      if (!(roomName in roomsLib.templates)) {
        throw new Error(
          `[rooms.append] append failed: the room ${roomName} does not exist!`
        );
      }
    ...
    prepend(roomName, params) {
      if (!(roomName in roomsLib.templates)) {
        throw new Error(
          `[rooms] prepend failed: the room ${roomName} does not exist!`
        );
      }
      const room = new Room(roomsLib.templates[roomName], false);
      if (params) {
    ...
    merge(roomName) {
      if (!roomsLib.current) {
        throw new Error(
          "[rooms.merge] Cannot merge in a room before the main one is created"
        );
      }
      if (!(roomName in roomsLib.templates)) {
        console.error(
          `[rooms] merge failed: the room ${roomName} does not exist!`
        );
        return false;
      }
      const generated = {
  ...
  var updateViewport = () => {
    ...
    let pixelScaleModifier = settings.highDensity
      ? window.devicePixelRatio || 1
      : 1;
    if (mode === "fastScale" || mode === "fastScaleInteger") {
      pixelScaleModifier = 1;
    }
    const kw = window.innerWidth / rooms_default.current.viewWidth,
      kh = window.innerHeight / rooms_default.current.viewHeight;
    ...
    if (mode === "expand") {
      ...
      if (mode === "scaleFill") {
        canvasWidth = Math.ceil(
          rooms_default.current.viewWidth * kw * pixelScaleModifier
        );
        canvasHeight = Math.ceil(
          rooms_default.current.viewHeight * kh * pixelScaleModifier
        );
        cameraWidth = window.innerWidth / k;
        cameraHeight = window.innerHeight / k;
      } else {
        canvasWidth = Math.ceil(
          rooms_default.current.viewWidth * k * pixelScaleModifier
        );
        canvasHeight = Math.ceil(
          rooms_default.current.viewHeight * k * pixelScaleModifier
        );
    ...
    if (pixiApp.view.style) {
      pixiApp.view.style.width =
        Math.ceil(canvasWidth / pixelScaleModifier) + "px";
      pixiApp.view.style.height =
        Math.ceil(canvasHeight / pixelScaleModifier) + "px";
    }
    if (camera_default) {
      const oldWidth = camera_default.width,
        oldHeight = camera_default.height;
  ...
  var CtAction = class {
    ...
    addMethod(code, multiplier) {
      if (this.methodCodes.indexOf(code) === -1) {
        ...
        throw new Error(
          `[ct.inputs] An attempt to add an already added input "${code}" to an action "${this.name}".`
        );
      }
    }
    /**
    ...
    setMultiplier(code, multiplier) {
      ...
      if (ind !== -1) {
        ...
        console.warn(
          `[ct.inputs] An attempt to change multiplier of a non-existent method "${code}" at event ${this.name}`
        );
        console.trace();
      }
    }
  ...
  var inputsLib = {
    ...
    addAction(name, methods) {
      if (name in actionsLib) {
        throw new Error(
          `[ct.inputs] An action "${name}" already exists, can't add a new one with the same name.`
        );
      }
  ...
  PIXI.particles.Particle.prototype.isInteractive = () => false;
  {
    ...
    PIXI.particles.PropertyNode.createList = (data) => {
      ...
      const first = (node = new PIXI.particles.PropertyNode(
      ...
      if (array.length > 1) {
        for (let i = 1; i < array.length; ++i) {
          ...
          node.next = new PIXI.particles.PropertyNode(
  ...
  var EmitterTandem = class extends PIXI.Container {
    ...
    update() {
      ...
      if (
        (this.appendant && this.appendant.destroyed) ||
        this.kill ||
      ...
      const s =
        (this.isUi
          ...
          : PIXI.Ticker.shared.deltaMS) / 1e3;
    ...
    stop() {
      if (this.stopped) {
        console.trace(
          "[emitters] An attempt to stop an already stopped emitter tandem. Continuing\u2026"
        );
        return;
      }
      this.stopped = true;
    ...
    updateFollow() {
      ...
      for (const emitter of this.emitters) {
        emitter.updateOwnerPos(
          this.follow.x + delta.x,
          this.follow.y + delta.y
        );
  ...
  var emittersLib = {
    ...
    fire(name, x, y, settings2) {
      if (!(name in emittersLib.templates)) {
        throw new Error(
          `[emitters] An attempt to create a non-existent emitter ${name}.`
        );
      }
      ...
      if (!opts.room) {
        if (!rooms_default.current) {
          throw new Error(
            "[emitters.fire] An attempt to create an emitter before the main room is created."
          );
        }
    ...
    append(parent, name, settings2) {
      ...
      if (!(parent instanceof PIXI.Container)) {
        throw new Error(
          "[emitters] Cannot attach an emitter as the specified parent cannot have child elements."
        );
      }
      const opts = Object.assign({}, defaultSettings, settings2);
      const tandem = new EmitterTandem(emittersLib.templates[name], opts);
      ...
      if (!("addChild" in parent)) {
        ...
        throw new Error(
          "[emitters] An attempt to append an emitter to an entity that doesn't support children."
        );
      }
...
${ev.error?.stack ?? "(no stack available)"}`;
  ...
  var meta = [{ name: "sewage", author: "chc", site: "", version: "0.0.0" }][0];
  var currentViewMode = "scaleFit";
  var currentHighDPIMode = Boolean([true][0]);
  var settings = {
  ...
  var loading;
  {
    ...
    const loop = () => {
      ...
      for (let i = 0, li = stack.length; i < li; i++) {
        templates_default.isCopy(stack[i]) &&
          templates_default.beforeStep.apply(stack[i]);
        stack[i].onStep.apply(stack[i]);
        templates_default.isCopy(stack[i]) &&
          templates_default.afterStep.apply(stack[i]);
      }
      for (const item of pixiApp.stage.children) {
        if (!(item instanceof Room)) {
          continue;
      ...
      for (let i = 0, li = stack.length; i < li; i++) {
        templates_default.isCopy(stack[i]) &&
          templates_default.beforeDraw.apply(stack[i]);
        stack[i].onDraw.apply(stack[i]);
        templates_default.isCopy(stack[i]) &&
          templates_default.afterDraw.apply(stack[i]);
...

  {
    ...
    templates.templates["ecoli-cartoon"] = {
      name: "ecoli-cartoon",
      depth: 0,
      ...
      texture: "ecoli-cartoon",
      animationFPS: 30,
      playAnimationOnStart: false,
      loopAnimation: true,
      behaviors: JSON.parse("[]"),
      onStep: function () {
        /* template ecoli-cartoon — core_OnStep (On frame start event) */
        {
          /**
           * Move the ship
          ...
          if (this.y < camera.height / 2) {
            // Have the ship crossed the middle line?
            this.y = camera.height / 2; // Go back to the middle line
          }
          if (this.y > camera.height) {
            // Have the ship crossed the bottom border?
            this.y = camera.height; // Go back to the bottom border
          }

          this.move();
        }
        /* template ecoli-cartoon — place_collisionCGroup (Collision with a group event) */
        {
          ...
          if (templates.valid(other)) {
            if (rooms.current.digest != "消化中……") {
              if (rooms.current.energy >= this.dict[other.template]) {
                ...
                rooms.current.energy -= this.dict[other.template];
                this.scale.x = this.scale.y =
                  (100 - rooms.current.energy) / 100 + 1;
                var ion = this.name[other.template];
                rooms.current.tasty = random.dice(
                  ion + " 真好吃！",
                  ion + " 真香呀！",
                  ion + " 太好吃了！",
                  "我还想再吃一个 " + ion + "！",
                ...
                if (rooms.current.score >= MAX_SCORE) {
                  ...
                  MessageBox(
                    "胜利",
                    "恭喜，你赢了！<br>经过小细菌的努力，河水中的重金属污染得到了治理，河流的生态环境得到了很大的改善！",
                    "返回主页",
                    undefined,
                    "ecoli_^^D"
                  );
                ...
                rooms.current.energyInsufficient = "能量不够了哦！";
                setTimeout(() => {
        ...
        /* template ecoli-cartoon — core_OnActionPress (OnActionPress event) */

        if (actions["Digest"].pressed) {
      ...
      onDestroy: function () {},
      onCreate: function () {
        /* template ecoli-cartoon — core_OnCreate (On create event) */
        {
          this.dict = {
            Ag: 10,
            Cd: 8,
    ...
    templates.list["ecoli-cartoon"] = [];

    templates.templates["Ag"] = {
      name: "Ag",
      depth: 0,
      blendMode: PIXI.BLEND_MODES.NORMAL,
      visible: true,
      ...
      behaviors: JSON.parse("[]"),
      onStep: function () {
        /* template Ag — core_OnStep (On frame start event) */
        {
          this.move();
          if (this.x > camera.width + 80 || this.x < -80) {
            this.kill = true;
          }
        }
      },
      ...
      onDestroy: function () {},
      onCreate: function () {
        /* template Ag — core_OnCreate (On create event) */
        {
          this.speed = random.range(20, 100);
          this.direction = 0;
          if (random.dice(0, 1)) {
            this.direction = 180;
            this.y = camera.width + 50;
          }
    ...
    templates.templates["Cr"] = {
      ...
      behaviors: JSON.parse("[]"),
      onStep: function () {
        /* template Cr — core_OnStep (On frame start event) */
        {
          this.move();
          if (this.x > camera.width + 80 || this.x < -80) {
            this.kill = true;
      ...
      onDestroy: function () {},
      onCreate: function () {
        /* template Cr — core_OnCreate (On create event) */
        {
          this.speed = random.range(40, 160);
          this.direction = 0;
          if (random.dice(0, 1)) {
            this.direction = 180;
            this.y = camera.width + 50;
          }
          this.scale.x = this.scale.y = 0.22;
        }
      },
      extends: {
        cgroup: "food",
    ...
    templates.templates["Cd"] = {
      ...
      behaviors: JSON.parse("[]"),
      onStep: function () {
        /* template Cd — core_OnStep (On frame start event) */
        {
          this.move();
          if (this.x > camera.width + 80 || this.x < -80) {
            this.kill = true;
      ...
      onDestroy: function () {},
      onCreate: function () {
        /* template Cd — core_OnCreate (On create event) */
        {
    ...
    templates.templates["Cu"] = {
      ...
      behaviors: JSON.parse("[]"),
      onStep: function () {
        /* template Cu — core_OnStep (On frame start event) */
        {
          this.move();
          if (this.x > camera.width + 80 || this.x < -80) {
            this.kill = true;
      ...
      onDestroy: function () {},
      onCreate: function () {
        /* template Cu — core_OnCreate (On create event) */
        {
          this.speed = random.range(50, 200);
          this.direction = 0;
          if (random.dice(0, 1)) {
          ...
          this.scale.x = this.scale.y = 0.14;
        }
      },
      extends: {
        cgroup: "food",
    ...
    templates.templates["Ni"] = {
      ...
      behaviors: JSON.parse("[]"),
      onStep: function () {
        /* template Ni — core_OnStep (On frame start event) */
        {
          this.move();
          if (this.x > camera.width + 80 || this.x < -80) {
            this.kill = true;
      ...
      onDestroy: function () {},
      onCreate: function () {
        /* template Ni — core_OnCreate (On create event) */
        {
          this.speed = random.range(50, 200);
          this.direction = 0;
          if (random.dice(0, 1)) {
          ...
          this.scale.x = this.scale.y = 0.17;
        }
      },
      extends: {
        cgroup: "food",
    ...
    templates.templates["New Style"] = {
      ...
      behaviors: JSON.parse("[]"),
      onStep: function () {
        /* template New Style — core_OnStep (On frame start event) */
        {
          var tmin = Math.floor(this.sec / 60),
            tsec = this.sec % 60;
          this.text =
            "时间：" +
            ((tmin < 10 ? "0" : "") +
            ...
            rooms.current.score +
            "    能量剩余：" +
            rooms.current.energy;
        }
      },
      onDraw: function () {},
      onDestroy: function () {},
      onCreate: function () {
        /* template New Style — core_OnCreate (On create event) */
        {
          this.sec = 0;
    ...
    templates.list["New Style"] = [];

    templates.templates["New Style1"] = {
      name: "New Style1",
      depth: 0,
      blendMode: PIXI.BLEND_MODES.NORMAL,
      visible: true,
      ...
      textStyle: "New Style1",
      defaultText: "",
      behaviors: JSON.parse("[]"),
      onStep: function () {
        /* template New Style1 — core_OnStep (On frame start event) */
        {
          this.text = rooms.current.energyInsufficient;
        }
      },
      onDraw: function () {},
      onDestroy: function () {},
    ...
    templates.list["New Style1"] = [];

    templates.templates["tasty"] = {
      name: "tasty",
      depth: 0,
      blendMode: PIXI.BLEND_MODES.NORMAL,
      visible: true,
      ...
      behaviors: JSON.parse("[]"),
      onStep: function () {
        /* template tasty — core_OnStep (On frame start event) */
        {
          this.text = rooms.current.tasty;
        }
      },
      onDraw: function () {},
      onDestroy: function () {},
    ...
    templates.list["tasty"] = [];

    templates.templates["digest_timer"] = {
      name: "digest_timer",
      depth: 0,
      blendMode: PIXI.BLEND_MODES.NORMAL,
      visible: true,
      ...
      behaviors: JSON.parse("[]"),
      onStep: function () {
        /* template digest_timer — core_OnStep (On frame start event) */
        {
          this.text = rooms.current.digest;
        }
      },
      onDraw: function () {},
      onDestroy: function () {},
    ...
    rooms.templates["main"] = {
      ...
      behaviors: JSON.parse("[]"),
      objects: JSON.parse(
        '[{"x":448,"y":64,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"New Style"},{"x":704,"y":120,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"New Style1"},{"x":448,"y":120,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"tasty"},{"x":704,"y":512,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"ecoli-cartoon"},{"x":448,"y":238,"opacity":1,"tint":16777215,"scale":{"x":1,"y":1},"rotation":0,"exts":{},"customProperties":{},"template":"digest_timer"}]'
      ),
      bgs: JSON.parse(
        '[{"texture":"bg","depth":0,"exts":{"movementX":0,"movementY":0,"parallaxX":1,"parallaxY":1,"repeat":"repeat","scaleX":0.68,"scaleY":0.68,"shiftX":0,"shiftY":0}}]'
      ),
      tiles: JSON.parse("[]"),
      ...
      onStep() {
        ...
        if (this.timer1 > 0 && this.timer1 <= (false ? u.timeUi : u.time)) {
          ...
          // asteroid timer
          this.timer1 = random.range(0.1, 0.8);
          templates.copy(
            random.dice(
              "Ag",
              "Cd",
            ...
            0,
            camera.height / 2 + random(camera.height / 2)
          );
        } else {
  ...
  const place = (function ctPlace() {
    ...
    const getSSCDShapeFromRect = function (obj) {
      ...
      if (obj.angle === 0) {
        position.x -= scaleX > 0 ? shape.left * scaleX : -scaleX * shape.right;
        position.y -= scaleY > 0 ? shape.top * scaleY : -shape.bottom * scaleY;
    ...
    const getSSCDShapeFromLine = function (obj) {
      ...
      if (obj.angle !== 0) {
        ...
        return new SSCD.Line(
          new SSCD.Vector(obj.x + x1, obj.y + y1),
          new SSCD.Vector(x2 - x1, y2 - y1)
        );
      }
      return new SSCD.Line(
        new SSCD.Vector(
    ...
    const templateNameFilter = (target, other, template) =>
      other.template === template;
    const cgroupFilter = (target, other, cgroup) =>
      !cgroup || cgroup === other.cgroup;
    ...
    const place = {
      ...
      drawDebugGraphic(absolute) {
        ...
        if (shape instanceof SSCD.Rectangle) {
          ...
          if (!absolute) {
            ...
            g.moveTo(
              shape.__points[0].x + this.x,
              shape.__points[0].y + this.y
            );
            for (let i = 1; i < shape.__points.length; i++) {
              g.lineTo(
                shape.__points[i].x + this.x,
                shape.__points[i].y + this.y
              );
            }
          }
          ...
          if (!absolute) {
            g.moveTo(shape.__position.x, shape.__position.y).lineTo(
          ...
          // Treat as a point
          g.moveTo(-16, -16).lineTo(16, 16).moveTo(-16, 16).lineTo(16, -16);
        } else {
      ...
      enableTilemapCollisions(tilemap, exactCgroup) {
        ...
        if (tilemap.addedCollisions) {
          throw new Error(
            "[place] The tilemap already has collisions enabled."
          );
        }
      ...
      traceRect(rect, cgroup, getAll) {
        ...
        if (
          Math.abs(rect.width) > place.gridX ||
          Math.abs(rect.height) > place.gridY
        ) {
          oversized = true;
        }
        const shape = {
      ...
      traceCircle(circle, cgroup, getAll) {
        let oversized = false;
        if (
          circle.radius * 2 > place.gridX ||
          circle.radius * 2 > place.gridY
        ) {
          oversized = true;
        }
        const shape = {
    ...
    Object.defineProperty(templates.CopyProto, "moveContinuous", {
      value: function (cgroup, precision) {
        if (this.gravity) {
          this.hspeed +=
            this.gravity * u.time * Math.cos((this.gravityDir * Math.PI) / 180);
          this.vspeed +=
            this.gravity * u.time * Math.sin((this.gravityDir * Math.PI) / 180);
        }
        return place.moveAlong(
          this,
          this.direction,
          this.speed * u.time,
          cgroup,
  ...
  const pointer = (function mountCtPointer() {
    ...
    var handleMove = function (e) {
      ...
      if (
        ...
        !(e.target.closest && e.target.closest(".ct-Errors"))
      ) {
        e.preventDefault();
      }
    ...
    const pointer = {
      ...
      updateGestures() {
        ...
        if (pointer.down.length > 1) {
          const events = [pointer.down[0], pointer.down[1]].sort(
            (a, b) => a.id - b.id
          );
          angle = u.pdn(events[0].x, events[0].y, events[1].x, events[1].y);
          distance = u.pdc(events[0].x, events[0].y, events[1].x, events[1].y);
        }
        ...
        for (const button in buttonMappings) {
          ...
          for (const p of pointer.down) {
            // eslint-disable-next-line no-bitwise
            if (
              (p.buttons & buttonMappings[button]) ===
  ...
  const vkeys = (function ctVkeys() {
    return {
      button(options) {
        ...
        if (
          typeof options.x === "function" ||
          typeof options.y === "function"
        ) {
    ...
    text(text) {
      ...
      for (let i = 0; i < text.length; i++) {
        if (text[i] === "{") {
          ...
          if (!leaf) {
            throw new Error(
              `Unbalanced braces. Faced an extra closing bracket at pos ${i}.`
            );
          }
          leaf.end = i;
          ...
          text =
            text.slice(0, leaf.start) + randomized + text.slice(leaf.end + 1);
          i -= leaf.end - leaf.start + 1 - randomized.length;
        }
      }
      if (bracketGroups.length > 0) {
        throw new Error(
          `Unbalanced braces. Faced an extra opening bracket at pos ${
            bracketGroups.pop().start
          }.`
        );
      }
      return text;
